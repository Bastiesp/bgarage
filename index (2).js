...
// Cloudinary — opcional: si no hay vars configuradas, las fotos quedan deshabilitadas
...
app.post('/api/historial/:id/fotos', authMiddleware, async (req, res) => {
  if (!process.env.CLOUDINARY_CLOUD_NAME)
    return res.status(503).json({ error: 'Cloudinary no configurado en variables de entorno' });
  try {
