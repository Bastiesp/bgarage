...
const fichaHistorialSchema = new mongoose.Schema({
  ...
  descripcion:      { type: String, default: '' },
  codigosDTC:       { type: [String], default: [] },
  items:            { type: [itemSchema], default: [] },
  // Costos (editable para cálculo de utilidad)
...
// ═══════════════════════════════════════════════════════════════════════════
// SCHEMA META COMERCIAL
// ═══════════════════════════════════════════════════════════════════════════
const metaComercialSchema = new mongoose.Schema({
  tipo:    { type: String, enum: ['semanal', 'mensual'], required: true },
  periodo: { type: String, required: true, index: true }, // 'YYYY-MM-DD' lunes o 'YYYY-MM'
...
const MetaComercial  = mongoose.model('MetaComercial',  metaComercialSchema);
...
// Actualizar ficha (DTC, notas, tipo, descripcion, km, costoRepuestos)
app.put('/api/historial/:id', authMiddleware, async (req, res) => {
  try {
    // Solo proteger campos de origen inmutables
    const { reparacionId, patente, marca, modelo, anio, cliente, telefono,
            numeroReparacion, fechaIngreso, items, fotos, ...body } = req.body;
...
// ─────────────────────────────────────────────────────────────────────────────
// RUTAS: METAS COMERCIALES
// ─────────────────────────────────────────────────────────────────────────────
app.get('/api/metas', authMiddleware, async (req, res) => {
  try {
    const docs = await MetaComercial.find().sort({ periodo: -1 }).lean();
    res.json(docs);
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// Upsert — crea o actualiza meta por tipo+periodo
app.post('/api/metas', authMiddleware, async (req, res) => {
  try {
    const { tipo, periodo, monto } = req.body;
    if (!tipo || !periodo) return res.status(400).json({ error: 'Faltan tipo o periodo' });
    const doc = await MetaComercial.findOneAndUpdate(
      { tipo, periodo },
      { $set: { monto: Number(monto) || 0 } },
...
app.delete('/api/metas/:id', authMiddleware, async (req, res) => {
  try {
    await MetaComercial.findByIdAndDelete(req.params.id);
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
